...
function createTableBody(data) {
  ...
  data.forEach((rowData) => {
    ...
    Object.entries(rowData).forEach(([key, cellData]) => {
      ...
      if (
        !isEmbed &&
        cellData !== STATUS_CODES.exp &&
        cellData !== STATUS_CODES.na &&
...
async function initializeVersionTable(container) {
  try {
    const response = await fetch(
      "https://raw.githubusercontent.com/TheBossMagnus/Thunder/refs/heads/main/versions.json",
    );
    const data = await response.json();
    const processedData = processStatusCodes(data);

    const fetchedData = processedData;
    let filteredData = showAllVersions
      ? fetchedData
      : filterDataForReducedMode(fetchedData);
    ...
    if (!isEmbed) {
      ...
      switchModeButton.addEventListener("click", () => {
        ...
        switchModeButton.textContent = showAllVersions
          ? "Hide unsupported versions"
          : "Show unsupported versions";
        container.innerHTML = "";
        const newData = showAllVersions
          ? fetchedData
          : filterDataForReducedMode(fetchedData);
